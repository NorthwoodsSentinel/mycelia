...
Define all TypeScript types and interfaces shared across the codebase. Every other component imports from this file.

## Location

```
mycelia/src/types.ts
```
...
- [ ] All entity types match D1 schema columns exactly
- [ ] All union types match CHECK constraints in schema
- [ ] API input types cover all endpoint request bodies
- [ ] Env type includes all Cloudflare bindings

## Notes

- `HelpRequest` is named to avoid collision with the global `Request` type
- The `detail` field on AuditLogEntry is stringified JSON — parse as needed
- All timestamps are ISO 8601 strings (D1 stores TEXT)
