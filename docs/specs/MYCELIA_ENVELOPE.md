...
**Delegation is its own authorization scheme (re-audit round 2, 2026-08-24).** A delegate never holds the root's bearer. It sends `Authorization: Delegated <root_agent_id>` plus `Delegation: base64url(JSON array of links)` plus a `DPoP` proof of the LEAF key with no `ath` claim (there is no bearer to bind to; the chain binds the key). A bearer request that carries a `Delegation` header is refused (`DELEG_WITH_BEARER`); a `Delegated` request without a chain or without a proof is refused (`DELEG_REQUIRED` / `POP_REQUIRED`) in every mode. There is no shadow arm for delegation: the chain and the leaf proof either verify or the request fails. Chain rules, unchanged: each link is `{ body: { delegated_by, delegated_to, to_jwk, scope[], exp, depth, max_depth, nonce }, sig }`, link 0 signed by the root agent's bound key (the root is pinned from the agent row, never read from the chain). Scope may only narrow (hierarchical `a:*`), `exp` may only shrink, depth is contiguous and bounded, every signature verifies under the previous link's key. The presenter proves possession of the LEAF key with `DPoP`. Delegated principals are default-deny: only `POST /v1/requests/:id/claims` and `POST /v1/requests/:id/responses` accept them, with required scope `bus:respond`; everything else answers `DELEG_ROUTE_NOT_DELEGABLE`, a scope miss answers `DELEG_SCOPE_DENIED`, and key binding is never delegable.
