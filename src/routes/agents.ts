...
agents.post('/me/pop-key', requireAgentKey, async (c) => {
  ...
  try {
    ...
    try { await writePopAudit(c.env.DB, { agent_id: auth.agent_id, outcome: 'audit_gap', reason: 'bind_completion_audit_failed', htm: c.req.method, htu: c.req.url, arm: nextMode, jkt: newJkt }); } catch (e2) { console.error('[pop-key] audit_gap fallback ALSO failed', String(e2)); }
    return c.json(success({ agent_id: auth.agent_id, pop_jkt: newJkt, pop_bound_at: boundAt, pop_mode: nextMode, rotated: !!row?.pop_jkt, audit_incomplete: true }), 202);
  }
