...
async function computeDrift(db: D1Database, kv: KVNamespace): Promise<DriftSummary> {
  ...
  const [totalReqRow, missingScopeRow, targetedRow, sealedRefRow] = await Promise.all([
    ...
    // Matches both legacy ('SACRED'/'sacred') and renamed ('sealed') refusal markers.
    db.prepare(`SELECT COUNT(*) as count FROM audit_log WHERE created_at >= ? AND (detail LIKE '%SACRED%' OR detail LIKE '%sealed%')`).bind(now24).first<{ count: number }>(),
  ]);
